...
@section('content')
    <div class="card-body">
        ...
        <div class="row ">
            {!!Form::label('midt','Upload MIDT Excel',['class'=>'col-form-label col-lg-2 require']) !!}

            <div class="form-group row col-lg-5 ">
                {{Form::file('midt', ['class' =>' form-control','required'=>'required'])}}
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <table id="booking-table" class="table table-striped table-bordered mdl-data-table mt-2">
                        <thead>
                        <tr>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalTitle">MIDT</h5>
    ...
    <script>
        $(document).ready(function () {
            ...
            function fetch_data(pageid) {
                $.ajax({
                    url: "booking/fetch_data/?page=" + pageid,
                    method: "GET",
                    dataType: "json",
                    success: function (data) {
                        if (data.length > 0) {
            ...
            function drawTable(data) {
                ...
                for (var count = data.length - 1; count >= 0; count--) {
                    ...
                    html += '<td contenteditable class="column_name" data-column_name="sabre_bookings" data-id="' + data[count].id + '">' + data[count].sabre_bookings + '</td>';
                    html += '<td contenteditable class="column_name" data-column_name="amadeus" data-id="' + data[count].id + '">' + data[count].amadeus + '</td>';
                    html += '<td contenteditable class="column_name" data-column_name="travel_port" data-id="' + data[count].id + '">' + data[count].travel_port + '</td>';
                    html += '<td contenteditable class="column_name" data-column_name="others" data-id="' + data[count].id + '">' + data[count].others + '</td>';
                    html += '<td contenteditable class="column_name" data-column_name="ta_id" data-id="' + data[count].id + '">' + data[count].ta_id + '</td>';
                ...
                html += '<td contenteditable id="sabre_bookings"></td>';
                html += '<td contenteditable id="amadeus"></td>';
                html += '<td contenteditable id="travel_port"></td>';
                html += '<td contenteditable id="others"></td>';
            ...
            $(document).on('click', '#add', function () {
                ...
                var year = $('#year').text();
                var sabre_bookings = $('#sabre_bookings').text();
                var amadeus = $('#amadeus').text();
                var travel_port = $('#travel_port').text();
                var others = $('#others').text();
                var ta_id = $('#tid').text();
                var marketsharecommitment = $('#marketsharecommitment').text();
                var accountmanager = $('#accountmanager').text();
                if (ta_id != '' && month != '' && year != '' && sabre_bookings != '' && amadeus != '' && travel_port != '' && marketsharecommitment != '' && accountmanager != '') {
                    $.ajax({
        ...
        $('#booking-table').on('click', 'a#openShow', function (e) {
            e.preventDefault();
            $('#showData').modal('show');
        ...
        $('#excelExport').on('click', function () {
            ...
            var url = "{{route('booking.exportexcel')}}?" + $.param(query)
            window.location = url;
        });
