    ...
    <div class="card-body">
        <a href="#" id="printDoc"
           class="btn btn-primary btn-labeled btn-labeled-left btn-lg legitRipple float-right mt-2 mr-2"><b><i
                    class="icon-printer2"></i></b> Export Career</a>
        <div class="panel-heading mdl-color-text--red">Career</div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalTitle">Career</h5>
    ...
    <script>
        $(document).ready(function () {
            ...
            function fetch_data(pageid, emp_id) {
                ...
                $.ajax({
                    url: "career/fetch_data/?page=" + pageid,
                    method: "GET",
                    data: {
                        'emp_no': emp_id
                    },
            ...
            $(document).on('blur', '.column_name', function () {
                ...
                if (column_name === 'position')
                    var column_position = column_value;
                if (column_name === 'level')
                    var column_level = column_value;
                var id = $(this).data("id");

                if (column_value != '') {
                    $.ajax({
                        url: "{{ route('career.update') }}",
                        method: "POST",
                        data: {column_name: column_name, column_value: column_value,position: column_position,level: column_level, id: id, _token: _token},
                        success: function (data) {
                            hideErrorMessage();
                            if (data.indexOf("You do not have access to do that.") >= 0)
                                alert('You do not have access to do that.');
