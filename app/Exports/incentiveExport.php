...
class incentiveExport implements FromView
{
    use Exportable;
    ...
    {
//        return view('backend.incentive.printExcel', [
//            'data' => $this->data
//        ]);
        ...
        if (!empty($this->from) && !empty($this->to) && empty($this->ta_id)) {
            ...
            return view('backend.incentive.printExcel')->with('data', $data);
            ...
            $data = Incentive::where('ta_id', $this->ta_id)->orderBy('id', 'asc')->paginate(6);
            return view('backend.incentive.printExcel')->with('data', $data);
            ...
            $data = Incentive::where('ta_id', $this->ta_id)->whereRaw("STR_TO_DATE(CONCAT(year,'-',month, '-', 1), '%Y-%m-%d') >= ?", "$fromYear-$fromMonth-1")
            ...
            $data = Incentive::where('ta_id', $this->ta_id)->where('month', $fromMonth)->where('year', $fromYear)->orderBy('id', 'asc')->paginate(6);
            return view('backend.incentive.printExcel')->with('data', $data);
